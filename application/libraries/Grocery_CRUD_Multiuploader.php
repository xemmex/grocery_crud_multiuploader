...
class Grocery_CRUD_Multiuploader extends grocery_CRUD
{
	...
	*/
	...
	protected function set_callbacks($file_field)
	{
		...
		/*
		*	PHP >= 5.3 can use anonymous functions like this
		*	$this->callback_column($file_field,($value, $row){	
    		*		return $this->_get_files_this_key($row->{$this->primary_key});
		*	});
		*/
		...
		/* Callback before insert modify post array */
		$this->callback_before_insert(array($this, '_set_files'));

		/* Callback before update modify post array */
		$this->callback_before_update(array($this, '_set_files'));

		/* Callback before delete unlink files */
		$this->callback_before_delete(array($this,'_delete_this_key'));
			
		/* Callback on read state */
		// GC < 1.5 we have some issue...		
		switch(explode(".",grocery_CRUD::VERSION)[1])
		{
		   case   5:
	...
	{
		$this->_reset( func_get_args()[2]->name);
		...
		$html.=$this->JS(func_get_args()[2]->name);
	...
	function edit_upload_fied($value, $primary_key)
	{
		...
		$files = unserialize($this->ci->db->get_where(
...
";
	 ...
	 * @return      array
	 */
	function _set_files($post_array, $primary_key)
	{
